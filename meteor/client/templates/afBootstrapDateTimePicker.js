    ...
    valueIn: function(val, atts) {
        ...
        if (typeof timezoneId === "string") {
            ...
            if (val instanceof Date) {
                return moment(AutoForm.Utility.dateToNormalizedLocalDateAndTimeString(val, timezoneId), "YYYY-MM-DD[T]HH:mm:ss.SSS")
                    .toDate();
            }
        }
        return val;
...
if (data.value instanceof Date) {
  var currentDate = dtp.getDate();
  if (currentDate && currentDate.getTime() !== data.value.getTime()) {
      dtp.setDate(data.value);
  }
} else {
  dtp.setDate(); // clear
