Schemas.SARCAT._firstLevelSchemaKeys.map(function(d) {
    var schema = Schemas[d] ? Schemas[d]._schema : Schemas.SARCAT._schema[d];
    var obj = {}
    if (schema.type) {
        ...
        if (schema.allowedValues) {
            obj.allowedValues = schema.allowedValues;
        }
        if (schema.label) {
            obj.label = schema.label;
        }
        obj.optional = schema.optional ? schema.optional : false;
        ...
        _.each(schema, function(val, name) {
            ...
            if (val.allowedValues) {
                obj[name].allowedValues = val.allowedValues;
            }
            if (val.label) {
                obj[name].label = val.label;
            }
            obj[name].optional = val.optional ? val.optional : false;

            /*delete schema[name].autoform;
            delete schema[name].autoValue;
            delete schema[name].defaulValue
            delete schema[name].denyInsert;*/
        })
